...
class SecmasterTest(BaseTest):
    ...
    def test_secmaster_workspace_query(self):
        ...
        self.assertEqual(len(resources), 2, "Should return 2 workspaces according to VCR file")
    ...
    def test_secmaster_alert_query(self):
        ...
        self.assertEqual(len(resources), 2, "Should return 2 alerts according to VCR file")
    ...
    def test_secmaster_playbook_query(self):
        ...
        self.assertEqual(len(resources), 3, "Should return 3 playbooks according to VCR file")
            ...
            'filters': [
                {
                    'type': 'age',
                    'days': 90,
                    'op': 'gt'  # Alerts older than 90 days
                }
            ]
        }, session_factory=factory)
        resources = p.run()
    ...
    def test_secmaster_playbook_value_filter_enabled(self):
        ...
        for i, playbook in enumerate(resources):
            self.assertTrue(playbook['enabled'], "Filtered playbooks should all be enabled")
    ...
    def test_secmaster_playbook_value_filter_disabled(self):
        ...
        for i, playbook in enumerate(resources):
            self.assertFalse(playbook['enabled'], "Filtered playbooks should all be disabled")
    ...
    def test_secmaster_playbook_enable_action(self):
        ...
        self.assertEqual(playbook2['workspace_id'], 'workspace001', "Should have workspace ID")
