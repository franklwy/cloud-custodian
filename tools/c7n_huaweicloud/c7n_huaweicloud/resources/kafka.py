...
# Note: HuaweiCloud SDK also provides TagEntity, this might be for specific scenarios or compatibility.
class TagEntity:
...
class Kafka(QueryResourceManager):
    ...
    This class is responsible for discovering, filtering, and managing Kafka instance resources on HuaweiCloud.
    It inherits from QueryResourceManager, utilizing its capabilities to query and process resource lists.
    ...
    def augment(self, resources):
        ...
        This method is mainly used to convert HuaweiCloud API returned tag list format (usually a dictionary list containing 'key' and 'value' fields)
        to AWS compatible format (dictionary list containing 'Key' and 'Value' fields) used by Cloud Custodian internally.
        ...
        :return: Enhanced resource dictionary list, where tags are converted to AWS compatible format under 'Tags' key
        """
        for r in resources:
...
class KafkaSecurityGroupFilter(SecurityGroupFilter):
    ...
    Allow users to filter instances based on attributes of security groups used by Kafka instances (such as name, ID).
...
class KafkaAutoTagUser(HuaweiCloudBaseAction):
    ...
    **Important Note:** This action depends on resource data containing creator information (such as 'user_name' field here).
    ...
    Therefore, the effectiveness of this action depends on whether `QueryResourceManager` or its `augment` method can obtain and fill `user_name` field through other means
