...
class ReusableFeaturesTest(BaseTest):
    ...
    def test_action_tag(self):
        """Test tag action - Add new tag"""
        factory = self.replay_flight_data("dns_public_zone_action_tag")
        # Get Zone ID from dns_public_zone_action_tag
        # Verify VCR: Match the 'id' in dns_public_zone_action_tag
        zone_id_to_tag = "2c9eb1538a138432018a13yyyyy00001"
        # Verify VCR: Match the 'key' in the request body of dns_public_zone_action_tag
        tag_key = "newtag"
        # Verify VCR: Match the 'value' in the request body of dns_public_zone_action_tag
        tag_value = "newvalue"
        p = self.load_policy(
            {
                "name": "dns-action-tag",
        ...
        self.assertEqual(len(resources), 1)
        # Assert mainly verifies if the policy correctly filtered the target resource
        # Verify action success: Manually check VCR cassette dns_public_zone_action_tag
        # to confirm POST /v2/zones/{zone_id}/tags was called with the correct tag
        # key and value

    def test_action_remove_tag(self):
        """Test remove-tag action - Remove tag"""
        # Verify VCR: Zone 'public-removetag.example.com.' in
        # dns_public_zone_action_remove_tag should have tag: {"toremove": "value"}
        factory = self.replay_flight_data("dns_public_zone_action_remove_tag")
        # Get Zone ID from dns_public_zone_action_remove_tag
        # Verify VCR: Match the 'id' in dns_public_zone_action_remove_tag
        zone_id_to_untag = "2c9eb1538a138432018a13wwwww00001"
        # Verify VCR: Match the tag key to be removed in dns_public_zone_action_remove_tag
        tag_to_remove = "toremove"
        p = self.load_policy(
            {
        ...
        # Assert mainly verifies if the policy correctly filtered the target resource
        # Verify action success: Manually check VCR cassette
        # dns_public_zone_action_remove_tag to confirm
        # DELETE /v2/zones/{zone_id}/tags/{tag_key} was called

    def test_action_mark_for_op(self):
        """Test mark-for-op action"""
        factory = self.replay_flight_data("dns_public_zone_action_mark_for_op")
        # Get Zone ID from dns_public_zone_action_mark_for_op
        # Verify VCR: Match the 'id' in dns_public_zone_action_mark_for_op
        zone_id_to_mark = "2c9eb1538a138432018a13vvvvv00001"
        # Verify VCR: Match the 'key' in the request body of
        # dns_public_zone_action_mark_for_op
        tag_key = "c7n_status"
        op = "delete"
        # Verify VCR: Match the number of days in the value of the request body in
        # dns_public_zone_action_mark_for_op
        days = 1
        p = self.load_policy(
            {
        ...
        # Assert mainly verifies if the policy correctly filtered the target resource
        # Verify action success: Manually check VCR cassette
        # dns_public_zone_action_mark_for_op to confirm POST /v2/zones/{zone_id}/tags
        # was called, tag value contains 'delete@<timestamp>' and days=1

    def test_action_auto_tag_user(self):
        """Test auto-tag-user action"""
        # auto-tag-user depends on resource creation info, usually from event-driven mode.
        # In tests using only cassettes, this behavior cannot be simulated,
        # defaults to tagging "unknown".
        # Note: The corresponding cassette dns_public_zone_action_auto_tag_user
        # is currently missing.
        # Requires corresponding cassette (currently missing)
        ...
        # Cannot execute p.run() and verify results due to missing cassette
        # self.assertEqual(len(resources), 1) # Assuming the cassette has matching resources
        # Assert mainly verifies if the policy correctly filtered the target resource
